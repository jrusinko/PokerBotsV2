...
run_viewer_app <- function(log_data = NULL) {
  ...
  normalize_replay_data <- function(x) {
    ...
    # Case 0: result wrapper from demo_tournament_run().
    if (is.list(x) && !is.null(x$tournament_state)) {
      x <- x$tournament_state
    }
  ...
  build_replay_snapshots <- function(hand) {
    ...
    if (length(snaps) == 0) {
      ...
      players <- hand$ending_stack_summary %||% hand$stack_summary %||% hand$starting_stack_summary %||% list()
      fallback_snap <- list(
        step = as.integer(hand$action_count %||% 0L),
        snapshot_type = "hand_end",
        message = "Hand complete",
        hand_id = hand$hand_id %||% NA_character_,
        hand_number = hand$hand_number %||% NA_integer_,
        street = hand$final_street %||% "hand_end",
        pot = hand$pot %||% 0,
        board = hand$board %||% character(0),
        action_count = as.integer(hand$action_count %||% 0L),
        players = lapply(players, function(p) {
          list(
            player_id = p$player_id %||% NA_character_,
            name = p$player_name %||% p$name %||% p$player_id %||% NA_character_,
  ...
  hand_action_history <- function(hand) {
    ah <- hand$action_history %||% list()
    if (is.list(ah) && length(ah) > 0) {
      return(ah)
    }
    ah <- hand$broadcast_action_history %||% hand$detail_action_history %||% list()
    if (is.list(ah) && length(ah) > 0) {
      return(ah)
    }

    snaps <- get_hand_snapshots(hand)
    if (!is.list(snaps) || length(snaps) == 0) {
      return(list())
    }

    snapshot_actions <- list()
    for (snap in snaps) {
      candidates <- snap$action_history %||% snap$actions %||% list()
      if (is.list(candidates) && length(candidates) > length(snapshot_actions)) {
        snapshot_actions <- candidates
      }
      latest <- snap$latest_action %||% snap$last_action %||% NULL
      if (is.list(latest) && length(latest) > 0) {
        idx <- as.integer(snap$action_count %||% (length(snapshot_actions) + 1L))
        idx <- max(1L, idx)
        snapshot_actions[[idx]] <- latest
      }
    }

    if (length(snapshot_actions) > 0) {
      return(snapshot_actions[!vapply(snapshot_actions, is.null, logical(1))])
    }
  ...
  winner_display_data <- function(hand) {
    ...
    for (w in winners) {
      ...
      if (identical(w$win_type %||% "", "showdown_pot_award")) {
        eligible_seats <- w$eligible_seats %||% NULL
        if (is.null(eligible_seats)) {
          matching_awards <- Filter(function(a) {
            identical(a$type %||% "", "showdown_pot_award") &&
              identical(as.integer(a$pot_index %||% NA_integer_), as.integer(w$pot_index %||% NA_integer_))
          }, hand_action_history(hand))
          if (length(matching_awards) > 0) {
            eligible_seats <- matching_awards[[1]]$eligible_seats %||% NULL
          }
        }
        if (!is.null(eligible_seats) && length(unique(as.character(eligible_seats))) < 2L) {
          next
        }
  ...
  elimination_extra_hold_ms <- 1800L
  ...
  snapshot_speech_hold_ms <- function(hand, snapshot) {
    ...
    hold_ms <- max(vapply(texts, estimated_speech_ms, integer(1)), 0L)
    if (identical(snap_type, "elimination")) {
      hold_ms <- hold_ms + elimination_extra_hold_ms
    }
    hold_ms
  }
  ...
  intro_source_files <- list(
    "Jaymon" = "FinalStudentBots/Jaymon.R",
    "Joel" = "FinalStudentBots/Joel.R",
    "Nikola" = "FinalStudentBots/Nikola.R",
    "Mehdi" = "FinalStudentBots/Mehdi.R",
    "Nate" = "FinalStudentBots/Nate.R",
    "Mady" = "FinalStudentBots/mady.R",
    "Tara" = "FinalStudentBots/Tara.R",
    "Lucy" = "FinalStudentBots/Lucy.R",
    "Siena" = "FinalStudentBots/Siena.R",
    "Ruth" = "FinalStudentBots/Ruth.R",
    "King Rikki" = "reference_bots/GuestBots.R",
    "Hatch Bot" = "reference_bots/GuestBots.R",
    "Gearan up to beat you" = "reference_bots/GuestBots.R",
    "Sir Hu McBluff" = "reference_bots/GuestBots.R",
    "Talmage Bot" = "reference_bots/GuestBots.R",
    "Bot inSpector" = "reference_bots/GuestBots.R",
    "Khan you fold?" = "reference_bots/GuestBots.R",
    "Fordeing Ahead" = "reference_bots/GuestBots.R",
    "Maurice Hawkins" = "reference_bots/GuestBots.R",
    "Biermann" = "reference_bots/GuestBots.R"
  )

  intro_source_prefixes <- list(
    "Jaymon" = c("Jaymon"),
    "Joel" = c("Joel"),
    "Nikola" = c("Nikola"),
    "Mehdi" = c("Mehdi"),
    ...
    "Siena" = c("Siena"),
    "Ruth" = c("Ruth"),
    "King Rikki" = c("King Rikki"),
    "Hatch Bot" = c("Hatch Bot"),
    "Gearan up to beat you" = c("Gearan Bot"),
    "Sir Hu McBluff" = c("Hu Bot"),
    "Talmage Bot" = c("Talmage Bot"),
    "Bot inSpector" = c("Spector Bot"),
    "Khan you fold?" = c("Khan Bot"),
    "Fordeing Ahead" = c("Forde Bot"),
    "Maurice Hawkins" = c("Hawkins Bot"),
    "Biermann" = c("Biermann Bot", "Biermann")
  )

  intro_is_example_bot <- function(player) {
    name <- as_scalar_chr(player$player_name, "")
    name %in% intro_example_bot_names
  }

  intro_extract_source_strings <- function(path) {
    full_path <- normalizePath(file.path(getwd(), path), winslash = "/", mustWork = FALSE)
    if (!file.exists(full_path)) {
      return(character(0))
    }

    parsed <- try(parse(full_path, keep.source = TRUE), silent = TRUE)
    ...
    tokens <- parse_data$text[parse_data$token == "STR_CONST"]
    tokens <- tokens[nzchar(tokens)]
    if (length(tokens) == 0) {
      return(character(0))
    }

    values <- try(eval(parse(text = paste0("c(", paste(tokens, collapse = ", "), ")"))), silent = TRUE)
    if (inherits(values, "try-error")) {
      return(character(0))
    }

    unlist(strsplit(as.character(values), "\n", fixed = TRUE), use.names = FALSE)
  }

  intro_clean_source_line <- function(line) {
    line <- trimws(as_scalar_chr(line, ""))
    line <- gsub("\\\\n", " ", line, fixed = TRUE)
    line <- gsub("[[:space:]]+", " ", line)
    line <- strip_chatter_speaker_prefix(line)
    trimws(line)
  }
  ...
  hand_summary_text <- function(hand) {
    paste0(
      ...
      "Action count: ", as_scalar_chr(hand$action_count, length(hand_action_history(hand))), "\n",
  ...
  replay$hand_log <- lapply(replay$hand_log, function(hand) {
    elim_ids <- hand$eliminations_this_hand %||% character(0)
    if (length(elim_ids) > 0) {
      hand$had_elimination <- TRUE
      hand$for_tv <- TRUE
      reasons <- trimws(as_scalar_chr(hand$interest_reasons, ""))
      ...
      if (!is.finite(as_scalar_num(hand$interest_score, NA_real_))) {
  ...
  intro_source_chatter <- intro_source_chatter_lookup()
    ...
    shiny::tags$head(
      ...
      shiny::tags$script(shiny::HTML("
        (function() {
          ...
          function playToneWhenSpeechIdle(kind) {
            if (kind !== 'elimination' || !('speechSynthesis' in window)) {
            ...
            function waitAndPlay() {
              ...
              window.setTimeout(waitAndPlay, 150);
            }
            waitAndPlay();
          }
          ...
          function speakChatter(message) {
            ...
            function isEnglishVoice(voice) {
              var lang = voice && voice.lang ? String(voice.lang).toLowerCase() : '';
              var name = voice && voice.name ? String(voice.name).toLowerCase() : '';
              return /^en([_-]|$)/.test(lang) ||
                name.indexOf('english') >= 0 ||
                name.indexOf('united states') >= 0 ||
                name.indexOf('american') >= 0 ||
                name.indexOf('us ') >= 0;
            }
            ...
            function getBotVoiceProfiles(voices) {
              ...
              var botVoiceIndices = voices.map(function(_, i) { return i; }).filter(function(i) {
                return i >= 1 && i <= 21 && isEnglishVoice(voices[i]);
              });
              if (!botVoiceIndices.length) {
                botVoiceIndices = voices.map(function(_, i) { return i; }).filter(function(i) {
                  return i >= 1 && isEnglishVoice(voices[i]);
                });
              }
              if (!botVoiceIndices.length) {
                botVoiceIndices = voices.map(function(_, i) { return i; }).filter(function(i) {
          ...
          function speakElimination(message) {
            var attempts = 0;
            function waitThenPlay() {
              ...
              window.setTimeout(waitThenPlay, 150);
            }
            waitThenPlay();
          }
          ...
          if (window.Shiny) {
            ...
            Shiny.addCustomMessageHandler('viewer-play-sound', function(message) {
              ...
              playToneWhenSpeechIdle(kind === 'level' || kind === 'elimination' || kind === 'chatter' || kind === 'bot' ? kind : 'feature');
            });
            ...
            Shiny.addCustomMessageHandler('viewer-speak-elimination', function(message) {
              speakElimination(message || {});
            });
          }
        })();
      "))
  ...
  server <- function(input, output, session) {
    ...
    replay_step <- shiny::reactiveVal(1L)
    ...
    speak_broadcast_elimination <- function(text) {
      if (!isTRUE(input$broadcast_mode %||% FALSE)) {
        return(invisible(NULL))
      }
      if (!isTRUE(input$broadcast_speak_chatter %||% TRUE)) {
        play_broadcast_sound("elimination")
        return(invisible(NULL))
      }

      session$sendCustomMessage(
        "viewer-speak-elimination",
        list(text = as_scalar_chr(text, ""))
      )
      invisible(NULL)
    }
    ...
    current_step_index <- shiny::reactive({
      snaps <- current_snapshots()
      if (length(snaps) == 0) return(NA_integer_)
      idx <- suppressWarnings(as.integer(replay_step() %||% input$step_index %||% 1L))
      if (length(idx) != 1L || is.na(idx)) idx <- 1L
      max(1L, min(idx, length(snaps)))
    })

    current_snapshot_at_step <- function() {
      snaps <- current_snapshots()
      if (length(snaps) == 0) return(NULL)
      snaps[[current_step_index()]]
    }

    shiny::observeEvent(input$play_replay, {
      snaps <- current_snapshots()
      if (length(snaps) > 0) {
        current_idx <- suppressWarnings(as.integer(replay_step() %||% input$step_index %||% 1L))
        if (length(current_idx) != 1L || is.na(current_idx) || current_idx >= length(snaps)) {
          replay_step(1L)
    ...
    shiny::observe({
      ...
      if (!identical(advance_key, last_intro_advance_key())) {
        comment <- intro_comment_for_player(player, intro_chatter, intro_source_chatter)
    ...
    shiny::observeEvent(input$next_step, {
      ...
      current_idx <- replay_step() %||% input$step_index %||% 1L
      next_idx <- min(as.integer(current_idx) + 1L, length(snaps))
      replay_step(next_idx)
    ...
    shiny::observe({
      ...
      if (!identical(advance_key, isolate(last_replay_advance_key()))) {
    ...
    shiny::observeEvent(input$step_index, {
      snaps <- current_snapshots()
      if (length(snaps) > 0) {
        idx <- suppressWarnings(as.integer(input$step_index %||% 1L))
        if (length(idx) == 1L && !is.na(idx)) {
          bounded_idx <- max(1L, min(idx, length(snaps)))
          if (!identical(as.integer(isolate(replay_step())), as.integer(bounded_idx))) {
            replay_step(bounded_idx)
          }
        }
      }
      ...
      speak_broadcast_elimination(elimination_announcement_text(snap, current_hand()))
    }, ignoreInit = TRUE)
    ...
    output$snapshot_info <- shiny::renderText({
      snapshot_summary_text(
        snapshot = current_snapshot_at_step(),
    ...
    output$snapshot_players <- shiny::renderTable({
      df <- snapshot_to_player_df(current_snapshot_at_step())
...
estimate_broadcast_runtime <- function(log_data,
  ...
  snapshot_extra_seconds <- function(snapshot) {
    if (!isTRUE(include_speech_padding) || !isTRUE(speak_chatter)) {
      return(0)
    }
    if (identical(as_scalar_chr(snapshot$snapshot_type, ""), "elimination")) {
      return(as.numeric(elimination_extra_seconds))
    }
    0
  }
  ...
  if (n_hands == 0) {
    out <- list(
      ...
      replay_steps = 0,
      elimination_hands = 0,
      televised_comments_by_player = data.frame(
        player = character(0),
        comments = integer(0),
        words = integer(0),
        stringsAsFactors = FALSE
  ...
  comment_totals <- televised_comments_by_player(hand_log)
    ...
    feature_seconds <- if (featured) {
      step_seconds <- if (n_steps > 0) {
                vapply(snaps, function(snapshot) {
                  max(
                    replay_ms / 1000,
                    speech_seconds_for_words(snapshot_speech_words(hand, snapshot)) + snapshot_extra_seconds(snapshot)
                  )
                }, numeric(1))
      } else {
        numeric(0)
  ...
  out <- list(
    ...
    televised_comments_by_player = comment_totals,
    assumptions = list(
      ...
      elimination_extra_seconds = elimination_extra_seconds,
