...
jp_bot <- function(bot_input) {
  ...
  risk <- 0
  if (!is.null(bf)) {
    if (isTRUE(bf$monotone))  risk <- risk + 0.05
    if (isTRUE(bf$paired))    risk <- risk + 0.03
    if (bf$connectivity >= 2) risk <- risk + 0.03
  }
...
#
 bot_input_example <- build_bot_input(tourn)
 action <- jp_bot(bot_input_example)
 print(action)
#
